...
class DecisionTreeClassifier:
    """Class representing DecisionTreeClassifier"""

    def __init__(self, max_depth=100, min_samples_split=2, max_features=1.0):
        self.root = None
    ...
    def _get_potential_thresholds(x):
        x = np.sort(x)
        quantile_idxs = (np.arange(0, 1.05, 0.05) * (x.size-1)).astype(int)
    ...
    def _get_gini_impurity(x, y, threshold):
        ...
        yl_counts, yr_counts = np.bincount(yl), np.bincount(yr)
